...
import QuestionForm from "@/features/forms/components/QuestionForm.jsx";
// import { useQuizzes } from "@/quizzes_to_remove/quiz.context.jsx";

import useEditForms, {useIdQuiz} from "@/features/forms/api/useEditForm.js";
...
function QuizEditForm() {
    ...
    const { isError, isLoading, isValue: quiz } = useIdQuiz(id);
    const { mutate, isError: editError, isLoading: editLoading, isValue: editVal } = useEditForms(id);
    console.log('raw response:', quiz);
    useEffect(() => {
            if (quiz) {
                setQuizData(quiz);
            } else {
            ...
            questions: quizData.questions.map((question) =>
                question.id === id
                    ? {
                          ...
                          options: [
                              ...question.options,
                              { text: "", id: Date.now(), isCorrect: false },
                          ],
                      }
                    : question,
            ),
                ...
                {quizData.questions.map((question, questionIndex) => (
                    <QuestionForm
                        ...
                        onChangeOption={(e, optionId) =>
                            handleOptionChange(e, question.id, optionId)
                        }
                        onChangeRadio={(optionId) =>
                            handleCorrectChange(question.id, optionId)
                        }
